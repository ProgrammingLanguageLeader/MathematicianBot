...
<b-container class="pt-4">
  <b-form @submit="onSubmit">
    <b-form-group label="Fill in the following fields to find extremum" class="pt-2">
      <b-input-group prepend="function">
        <b-input
          v-model="extremaFunction"
          required
          placeholder="a function to find extremum"
        />
      </b-input-group>
...
export default {
  ...
  methods: {
    async onSubmit (event) {
      ...
      const requestString = `extrema ${this.extremaFunction} ${fromString} ${toString}`
      this.fetching = true
      try {
