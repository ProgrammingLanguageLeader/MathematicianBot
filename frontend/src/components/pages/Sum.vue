...
<b-container class="pt-4">
  <b-form @submit="onSubmit">
    <b-form-group label="Fill in the following fields to calculate sum" class="pt-2">
      <b-input-group prepend="sequence">
        <b-input
          v-model="sumFunction"
          required
          placeholder="a sequence to calculate sum"
        />
      </b-input-group>
...
export default {
  ...
  methods: {
    async onSubmit (event) {
      ...
      const requestString = `sum ${this.sumFunction}, ${this.independentVariable}=${this.fromNumber} to ${this.toNumber}`
      this.fetching = true
      try {
